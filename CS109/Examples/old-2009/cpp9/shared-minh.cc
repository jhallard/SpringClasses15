// $Id: shared-minh.cc,v 1.6 2009-03-10 19:13:11-07 - - $

//
// Shared (virtual) multiple inheritance.
// We use inline functions for brevity, not necessarily because it
// is a good idea.  This is one case where a ctor may invoke a
// grandparent.  Usually only parent invocation is permitted.
//
...
class base {
   public:
      base (): value (0) {
         info (*this); cout << ", base() value = " << value << endl;
      }
      base (int valuei): value (valuei) {
         info (*this); cout << ", base(int) value = " << value << endl;
      }
      virtual void print () {
...
class diamond: public derived, public derived2 {
   public:
      diamond (int i1, int i2, int i3, int i4, int i5):
                  base (i1), derived (i1, i2), derived2 (i3, i4),
                  bottom (i5) {
